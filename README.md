...
| **Sega SMS** | 1990 | 🟢 tileset 已取(libretro genesis-plus-gx headless dump VRAM+CRAM,色彩正確:藍水/綠林/金船/灰城堡);整合中 |

不只畫面 —— 連各版的**音效**也一併挖了出來:FM Towns 的攻擊 / 魔法 / 馬蹄(具名 `.SND`)、
X68000 的 OKI ADPCM 樣本、Amiga 的 Paula 8-bit 音效庫,三家三種 PCM 格式都轉成了現代 WAV。
在沒有喇叭的 headless 容器裡,怎麼確認自己解碼的是真音效而非一段雜訊?**把波形畫成圖,用眼睛驗收**
—— 數得出馬蹄拍、看得到方波,就知道全對了。過程與方法見 **[各平台音效抽取手記](docs/audio-extraction.md)**。
